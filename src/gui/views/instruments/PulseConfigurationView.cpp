...
PulseConfigurationView::PulseConfigurationView(
    ...
    , m_duty_cycle(graphics_factory
                       .createWidget("duty_cycle")
                       .fill(view_widget.entity, 32)
                       .addZPosition(view_widget.z()))
{
    m_duty_cycle.onDraw([this, instrument_index, song_model](graphics::Painter& painter) {
        const auto height = m_duty_cycle.height();
        const auto width = m_duty_cycle.width();
        auto duty_cycle = song_model->getInstrument(instrument_index).pulse().dutyCycle();
        SDL_Rect rectangle;
        rectangle.x = 0;
        rectangle.w = width;

        switch (duty_cycle) {
        case model::PulseInstrument::DutyCycle::Percent_12_5:
            rectangle.h = height / 8;
            break;
        case model::PulseInstrument::DutyCycle::Percent_25:
            rectangle.h = height / 4;
            break;
        case model::PulseInstrument::DutyCycle::Percent_50:
            rectangle.h = height / 2;
            break;
        case model::PulseInstrument::DutyCycle::Percent_75:
    ...
    m_duty_cycle.onPressed(ecs::Button::Left, [song_model, instrument_index](ecs::Button, int, int) {
        ...
        song_model->getInstrument(instrument_index)
            .pulse()
            .setDutyCycle(static_cast<model::PulseInstrument::DutyCycle>(
                (int(duty_cycle) + 1) % 4));
    });
}
