    ...
    const nlohmann::json& json_root)
{
    ...
    for (const auto& phrase : json_root["song"]["phrases"]) {
        ...
        ++phrase_index;
        if (phrase_index == model::Song::PHRASE_COUNT) {
            break;
        }
    }
}
    ...
    const nlohmann::json& json_root)
{
    ...
    for (const auto& instrument : json_root["song"]["instruments"]) {
        ...
        instrument_model.setType(
            getJsonValueOrDefault<model::Instrument::Type>(
                instrument,
                "type",
                model::Instrument::Type::None));

        auto& pulse_model = instrument_model.pulse();

        if (instrument.find("pulse") != instrument.end()) {
            auto& pulse = instrument["pulse"];
            pulse_model.setDutyCycle(
                getJsonValueOrDefault(
                    pulse,
                    "duty_cycle",
                    model::PulseInstrument::DutyCycle::Percent_12_5));

            pulse_model.setEnvelopeIncrease(getJsonValueOrDefault(pulse, "envelope_increase", false));
            pulse_model.setEnvelopePace(getJsonValueOrDefault(pulse, "envelope_pace", 0));
            pulse_model.setEnvelopeInitialVolume(getJsonValueOrDefault(pulse, "envelope_initial_volume", 0));
            pulse_model.setInitialLengthTimer(getJsonValueOrDefault(pulse, "initial_length_timer", 0));
            pulse_model.setLengthEnable(getJsonValueOrDefault(pulse, "length_enable", false));
            pulse_model.setSweepIncrease(getJsonValueOrDefault(pulse, "sweep_increase", false));
            pulse_model.setSweepPace(getJsonValueOrDefault(pulse, "sweep_pace", 0));
            pulse_model.setSweepSteps(getJsonValueOrDefault(pulse, "sweep_steps", 0));
        }
        ...
        if (instrument.find("wave") != instrument.end()) {
            ...
            wave_model.setLengthEnable(getJsonValueOrDefault(wave, "length_enable", false));
            wave_model.setLengthTimer(getJsonValueOrDefault(wave, "length_timer", 0));
    ...
    const nlohmann::json& json_root)
{
    ...
    for (const auto& track : json_root["song"]["tracks"]) {
        ...
        for (const auto& chain : track) {
            const auto chain_index = getJsonValue<size_t>(chain, "chain_index");
            if (chain_index) {
                track_model.setPhraseIndex(track_element_index, *chain_index);
            }

            ++track_element_index;
            if (track_element_index == model::Track::PHRASE_COUNT) {
                break;
            }
        }
        ++track_index;
        if (track_index == model::Song::TRACK_COUNT) {
            break;
        }
    }
}
    ...
    const std::filesystem::path& file_path)
{
    ...
    if (!json_root.contains("song")) {
        std::cerr << "Failed to parse json file, 'song' node is not present" << std::endl;
        return false;
    }

    if (!json_root["song"].contains("phrases")) {
        std::cerr << "Failed to parse json file: 'phrases' nodes are not present" << std::endl;
        return false;
    }

    if (!json_root["song"].contains("instruments")) {
        std::cerr << "Failed to parse json file: 'instruments' nodes are not present" << std::endl;
        return false;
    }
