    ...
    size_t target_frequency)
{
    ...
    // Verify DAC is on and we are currently playing a sound (after being triggered)
    // Verify also that volume is not muted
    ...
    const bool is_length_timer_enabled = m_audio_registers->getChannel3LengthEnable();
    ...
    for (size_t i = 0, end = samples.size(); i < end; ++i) {
        ...
        const auto output_value = (double(int(wave_value) - 8) / 7.) * volume_scaling;
