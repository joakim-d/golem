...
#include <golem/utils/shared_reference.h>
...
{
    template <typename... Args> friend class observable;
...
private:
    observable_handle(std::function<void()> unregister_callback)
...
public:
    ...
    {
        callbacks.push_back({ current_id, std::move(callback) });

        auto weak_ref
            = utils::weak_reference<observable<Args...>> { self_reference };

        observable_handle handle { [weak_ref, current_id = current_id]()
                                   {
                                       if (auto ref = weak_ref.lock())
                                       {
                                           ref->get().unregister_callback(
                                               current_id);
                                       }
                                   } };
...
private:
    void unregister_callback(observable_callback_id id)
    {
        auto callback_iterator = std::find_if(
            ...
            [id](const std::pair<
                 observable_callback_id, std::function<void(Args...)>>&
                     callback) { return id == callback.first; });
    ...
    const utils::shared_reference<observable<Args...>> self_reference;
    observable_callback_id current_id { 0 };
    std::list<std::pair<observable_callback_id, std::function<void(Args...)>>>
        callbacks;
};
}
