    ...
    , m_y_offset(0)
{
    ...
    for (int octave = 0; octave < 6; octave++)
    {
        ...
        for (auto white_key_index : white_key_indexes)
        {
            ...
            keys_rectangles[octave * 12 + white_key_index].y
                = (168 * octave) + i * (white_key_height + 1);
            i++;
        }
        for (auto black_key_index : black_key_indexes)
        {
            ...
            keys_rectangles[octave * 12 + black_key_index].y
                = (168 * octave) + black_key_index * 14;
        }
    }

    view_widget.onDraw(
        [keys_rectangles, view_widget, this](graphics::Painter& painter)
        {
            const int key_index_begin = (m_y_offset / (BLACK_KEY_HEIGHT));
            const int max_keys_to_draw
                = (view_widget.height() / BLACK_KEY_HEIGHT) + 1;
            ...
            for (int i = key_index_begin;
                 i < key_index_begin + keys_count_to_draw; ++i)
            {
    ...
    view_widget.onPressed(
        ...
        [this](ecs::Button, int x, int y)
        {
            ...
            m_use_cases.play_note.execute(
                0, domain::Channel::Channel1, domain::NoteFrequency(note));
        });

    view_widget.onReleased(
        ecs::Button::Left,
        [this](ecs::Button, int, int)
        { m_use_cases.stop_note.execute(domain::Channel::Channel1); });
}
...
{
    return (WHITE_KEY_HEIGHT + SPACING) * 7
        * OCTAVE_COUNT; // 7 white keys by octave * height
}
