    ...
    , m_y_offset(m_piano_roll_view.viewHeight() / 2)
{
    ...
    view_widget.onPositionChanged(
        [press_tracking, this, view_widget](ecs::Button, int x, int y)
        {
            ...
            const int max_y_offset = m_piano_roll_view.viewHeight()
                - view_widget.height() + SEQUENCER_HEADER_HEIGHT;
...
void MainView::onProgressionChanged(size_t tick_index, size_t note_index)
{
    m_sequencer_view.onProgressionChanged(tick_index, note_index);
}
