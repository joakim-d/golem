    ...
    , m_x_offset(0)
{
    ...
    for (size_t i = 0; i < 16; ++i)
    {
        ...
        current_widget.addFill(i % 2 == 0 ? firstColor : secondColor)
            .addSize(128, 16)
            .anchorTop(song_view_entity, ecs::Top)
            .addUpdateCallback(
                [current_widget, i, this]()
                {
        ...
        graphics_factory
            .createLabel(
                std::to_string(i + 1), 12, Style::fontPath(),
                Style::defaultColor())
    ...
    for (size_t i = 0; i < 16; ++i)
    {
        ...
        current_widget.addFill(i % 2 == 0 ? firstColor : secondColor)
            ...
            .onPressed(
                ecs::Button::Left,
                [this](ecs::Button, int x, int y) { trackClicked(0); });

        views::PatternView pattern_view { graphics_factory, current_widget,
                                          project_use_cases, 0, i };
    ...
    for (size_t i = 0; i < 16; ++i)
    {
        ...
        current_widget.addFill(i % 2 == 0 ? firstColor : secondColor)
            ...
            .onPressed(
                ecs::Button::Left,
                [this](ecs::Button, int x, int y) { trackClicked(1); });

        views::PatternView pattern_view { graphics_factory, current_widget,
                                          project_use_cases, 1, i };
    ...
    for (size_t i = 0; i < 16; ++i)
    {
        ...
        current_widget.addFill(i % 2 == 0 ? firstColor : secondColor)
            ...
            .onPressed(
                ecs::Button::Left,
                [this](ecs::Button, int x, int y) { trackClicked(2); });

        views::PatternView pattern_view { graphics_factory, current_widget,
                                          project_use_cases, 2, i };
    ...
    for (size_t i = 0; i < 16; ++i)
    {
        ...
        current_widget.addFill(i % 2 == 0 ? firstColor : secondColor)
            ...
            .onPressed(
                ecs::Button::Left,
                [this](ecs::Button, int x, int y) { trackClicked(3); });

        views::PatternView pattern_view { graphics_factory, current_widget,
                                          project_use_cases, 3, i };
        previous_entity = current_widget.entity;
    }
    ...
    song_view_widget.onPositionChanged(
        [mouse_state, this, song_view_widget](ecs::Button, int x, int y)
        {
            ...
            const auto max_delta
                = PHRASE_WIDTH * 16 - song_view_widget.width() + 64;
...
void SongView::onProgressionChanged(size_t tick_index, size_t note_index)
{
    constexpr double ticks_per_note = 16.;
    m_progression_bar.addPosition(
        65 + (double(tick_index) / ticks_per_note) * NOTE_WIDTH
