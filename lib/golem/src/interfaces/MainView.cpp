...
MainView::MainView(
    ...
    size_t track_index)
{
	ImGui::BeginChild("MainView", ImVec2 {-1, -1}, 0, ImGuiWindowFlags_NoScrollbar);
	...
	if (IsMouseChildDown(ImGuiMouseButton_Middle)) {
		...
		const float max_y_offset = PianoRollView::viewHeight()
								 - window_size.y
	...
	if (ImGui::IsKeyPressed(ImGuiKey_Space)) {
        use_cases.get_playback_info.execute().stopped
			? use_cases.play_song.execute(GuiState::instance().song_index)
