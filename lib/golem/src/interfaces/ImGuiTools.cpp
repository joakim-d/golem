...
void WindowDrawHelper::drawFilledRect(
	ImVec2 position,
	ImVec2 size,
	ImU32 color)
{
    m_draw_list->AddRectFilled(
		position + m_window_position, position + m_window_position + size, color);
}

void WindowDrawHelper::drawRect(
	ImVec2 position,
	ImVec2 size,
	ImU32 color)
{
	m_draw_list->AddRect(position + m_window_position, position + m_window_position + size, color);
}

void WindowDrawHelper::addText(
	ImVec2 position,
	ImU32 color,
	const char* text)
{
...
bool IsMouseChildClicked(ImGuiMouseButton button)
{
    ...
    return ImGui::IsMouseClicked(button)
		&& ImGui::IsMouseHoveringRect(window_position, window_position + ImGui::GetWindowSize());
}
...
ImColor getClickableItemColor(bool is_already_selected)
{
	if (ImGui::IsItemHovered()) {
		...
		return GuiState::instance().hovered_color;
    }
    if (is_already_selected)
		return GuiState::instance().selected_color;

	return GuiState::instance().idle_color;
}
