...
ApuEmulator::ApuEmulator(std::shared_ptr<IAudioOutputDevice> audio_output)
...
{
    m_audio_output->setGetSamplesCallback(std::bind(
        &ApuEmulator::getSamplesCallback, this, std::placeholders::_1,
        std::placeholders::_2));
...
void ApuEmulator::updateWaveRam(const std::array<uint8_t, 16>& wave_ram)
{
...
{
    const size_t sample_count = length
        / (m_audio_output->channelCount()
           * IAudioOutputDevice::byteCountForFormat(m_audio_output->format()));
    ...
    m_channel_three_emulator.getSamples(
        m_channel_three_samples, target_frequency);
