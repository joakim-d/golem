...
{
    const auto nr10 = m_register_values[NR10];
...
{
    const auto nr11 = m_register_values[NR11];
...
{
    const auto nr12 = m_register_values[NR12];
    return (nr12 >> 4);
}
...
{
    auto& nr13 = m_register_values[NR13];
    auto& nr14 = m_register_values[NR14];
    nr13 = period & 0xFF;
...
{
    const auto nr13 = m_register_values[NR13];
...
{
    const auto nr21 = m_register_values[NR21];
...
{
    const auto nr22 = m_register_values[NR22];
    return (nr22 >> 4);
}
...
{
    const auto nr24 = m_register_values[NR24];
    return (nr24 & 0x40);
}

void AudioRegisters::setChannel2Period(unsigned period)
{
    auto& nr23 = m_register_values[NR23];
    auto& nr24 = m_register_values[NR24];
    nr23 = period & 0xFF;
...
{
    const auto nr23 = m_register_values[NR23];
    const auto nr24 = m_register_values[NR24];
...
{
    const auto nr33 = m_register_values[NR33];
    const auto nr34 = m_register_values[NR34];
    return ((nr34 & 0b00000111) << 8) + nr33;
}

unsigned AudioRegisters::getChannel3OutputLevel() const
{
    const auto nr32 = m_register_values[NR32];
    return (nr32 & 0b01100000) >> 5;
}

bool AudioRegisters::getChannel3On() const
{
    return m_register_values[NR30] & 0b10000000;
}
...
{
    return m_register_values[NR31];
}

void AudioRegisters::setRegisterValue(Register reg, uint8_t value, uint8_t mask)
{
    auto register_value = m_register_values[reg] & (~mask);
    m_register_values[reg] = register_value | (value & mask);
}

void AudioRegisters::setWaveRam(const std::array<uint8_t, 16>& ram)
{
    m_wave_ram = ram;
}
