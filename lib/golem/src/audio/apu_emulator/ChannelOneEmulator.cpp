...
ChannelOneEmulator::ChannelOneEmulator(
...
void ChannelOneEmulator::getSamples(std::vector<float>& samples)
{
    ...
    const bool volume_increase
        = m_audio_registers->getChannel1EnvelopeDirection();
    const int envelope_sweep_pace
        = m_audio_registers->getChannel1EnvelopeSweepPace();
    const int sweep_pace = m_audio_registers->getChannel1SweepPace();

    std::optional<unsigned> volume_pace_ticks_target;
    std::optional<unsigned> sweep_ticks_target;
    const auto length_timer_ticks = CPU_FREQUENCY / 256;

    const bool is_length_timer_enabled
        = m_audio_registers->getChannel1LengthEnable();
    ...
    for (size_t i = 0; i < samples.size(); ++i)
    {
        ...
        if (volume_pace_ticks_target.has_value())
        {
            ...
            const unsigned volume_ticks_increment
                = m_volume_ticks / *volume_pace_ticks_target;
            m_volume_ticks %= *volume_pace_ticks_target;
            if (volume_increase)
            {
                m_volume = m_volume + volume_ticks_increment > 15
                    ? 15
                    : m_volume + volume_ticks_increment;
            }
            else
            {
                m_volume = volume_ticks_increment > m_volume
                    ? 0
                    : m_volume - volume_ticks_increment;
            }
        }
        if (sweep_ticks_target.has_value())
        {
            ...
            m_sweep_ticks %= *sweep_ticks_target;
            const bool increase
                = m_audio_registers->getChannel1SweepDirection();
            ...
            for (unsigned j = 0; j < sweep_ticks_increment; ++j)
            {
                ...
                new_period = increase
                    ? new_period + (new_period / std::pow(2, steps))
                    : new_period - (new_period / std::pow(2, steps));
        ...
        if (m_on)
        {
            current_value = m_audio_registers->getChannel1DutyCycleBitmask()
                    & (1 << m_waveform_index)
                ? volume
                : -volume;
        }
