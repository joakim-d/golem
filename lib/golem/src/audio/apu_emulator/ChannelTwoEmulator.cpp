...
void ChannelTwoEmulator::getSamples(std::vector<float>& samples)
{
    ...
    const bool volume_increase
        = m_audio_registers->getChannel2EnvelopeDirection();
    const int envelope_sweep_pace
        = m_audio_registers->getChannel2EnvelopeSweepPace();
    ...
    const bool is_length_timer_enabled
        = m_audio_registers->getChannel2LengthEnable();
    ...
    for (size_t i = 0; i < samples.size(); ++i)
    {
        ...
        if (m_on)
        {
            current_value = m_audio_registers->getChannel2DutyCycleBitmask()
                    & (1 << m_waveform_index)
                ? volume
                : -volume;
        }
